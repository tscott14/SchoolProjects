...
PASSCODE_HINTS=$(get_config $CONFIG_FILE 'PasscodeHints')
...
get_directions() {
    ...
    echo -ne "${land_north:+N}${land_south:+S}${land_east:+E}${land_west:+W}"
}
...
game_stage() {
    ...
    until [[ $player_input =~ $PASSCODE || $player_input =~ ':q!' ]]; do
        ...
        local magen_fg=$(tput setaf 5)
        local cyan_fg=$(tput setaf 6)
        local white_fg=$(tput setaf 7)
        local reset_tput=$(tput sgr 0)

        # Get available directions of travel for this land.
        local dirs_available=$(get_directions "$land_obj")
        ...
        local map_output=$(
            cat $ASCII_MAP_LAYOUTS/$land_layout_path |
                sed "s/N/${green_fg}N${white_fg}/g; 
                    s/E/${red_fg}E${white_fg}/g; 
                    s/S/${magen_fg}S${white_fg}/g; 
                    s/W/${cyan_fg}W${white_fg}/g" |
                sed "s/A/$name_centered/g; 
                    s/B/$middle_centered/g; 
                    s/C/$cell_centered/g"
        )
        ...
        # Print the hints already revieled.
        if [[ $HINTS =~ $CURR_LAND_ID ]]; then
            # HINTS=$(echo "$HINTS" | sort -t ' ' -V)
            local hint_highlight=$(tput smul)
            local colm_delim="\$$CURR_LAND_ID"
            local hint=$(echo -n $PASSCODE_HINTS | awk "{print $colm_delim};")
            echo -ne "\n${red_fg}You uncovered the clue \"${hint_highlight}$hint${reset_tput}${red_fg}\" from these lands previously!${white_fg}\n\n"
        fi
        ...
        while [[ $restate_prompt = $TRUE ]]; do
            ...
            if [[ $player_input =~ ^[S|s][E|e]([A|a][R|r][C|c][H|h])?$ ]]; then
                ...
                [[ $HINTS =~ $CURR_LAND_ID ]] || HINTS="$HINTS $CURR_LAND_ID "
                restate_prompt="$TRUE"
...
rungame() {
    ...
    # Main game loop
    trap "echo There are no easy escapes to the West! (Easy exit with :q!)" INT
    game_stage
    trap INT
